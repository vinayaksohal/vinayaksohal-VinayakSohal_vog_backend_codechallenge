...
{
    public class InCollectionContext:IDataRepository<Employee>
    {
        ...
        public List<Employee> GetList(params Expression<Func<Employee, object>>[] navigationProperties)
        {
            return employees;
        }

        public IEnumerable<Employee> GetAll(params Expression<Func<Employee, object>>[] navigationProperties)
        {
            return employees;
        }

        public IList<Employee> ListAll(Expression<Func<Employee, bool>> where, params Expression<Func<Employee, object>>[] navigationProperties)
        {
            return employees.AsQueryable().Where(where).ToList();
        }
    }
}
